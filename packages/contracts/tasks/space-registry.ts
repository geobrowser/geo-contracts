...
task(
  'space-registry:create-with-id',
  'Create a space with a specific ID (owner only)'
)
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    const spaceId = ethers.utils
      .formatBytes32String(taskArgs.spaceId)
      .slice(0, 34); // bytes16 = 0x + 32 chars
    ...
    if (owner !== signer.address) {
      throw new Error(
        `Signer ${signer.address} is not the owner. Owner is ${owner}`
      );
    }
    ...
    if (existingDao !== ethers.constants.AddressZero) {
      throw new Error(
        `Space ID ${spaceId} already exists with DAO ${existingDao}`
      );
    }
    ...
    // Create the space
    console.log('\nCreating space...');
    const tx = await registry.createSpaceWithId(
      daoSettings,
      pluginSettings,
      spaceId
    );
...
task('space-registry:transfer-ownership', 'Transfer ownership of SpaceRegistry')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (currentOwner !== signer.address) {
      throw new Error(
        `Signer ${signer.address} is not the current owner. Current owner is ${currentOwner}`
      );
    }

    if (currentOwner === taskArgs.newOwner) {
      console.log(
        '⚠️  New owner is the same as current owner. No transfer needed.'
      );
      return;
    }
    ...
    if (newOwnerAfterTransfer === taskArgs.newOwner) {
      console.log(
        ...
        newOwnerAfterTransfer
      );
    } else {
      console.log(
        '❌ Ownership transfer failed. Current owner is still:',
        newOwnerAfterTransfer
      );
    }
  });
...
task('space-registry:upgrade', 'Upgrade SpaceRegistry implementation')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (currentImpl.toLowerCase() === taskArgs.implementation.toLowerCase()) {
      console.log(
        '⚠️  New implementation is the same as current. No upgrade needed.'
      );
      return;
    }
